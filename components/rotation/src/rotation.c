...
void task_rotation(void *arg){
    (void)arg;

    const gpio_num_t gpio_pin = (gpio_num_t)arg;

    // create task variables
    const int send_rate_ms = (int)(1000.0 / (float)(TASK_ROTATION_SEND_RATE_Hz));
    const float send_rate_s = 1.0f / (float)TASK_ROTATION_SEND_RATE_Hz;
    ...
    for (;;) {
        if ((esp_log_timestamp() - timer_send_ms) >= send_rate_ms){
            ...
            if (err == ESP_OK) {
                ...
                float meters = (pulse_count / WHEEL_EDGES)*(60);
                rot.value = meters / send_rate_s; // final rpm

                ESP_LOGW(TAG, "rotation: %f, counts: %i", rot.value, pulse_count);

                // -----send spped data through esp-now to receiver-----
                system_global.rotation = rot.value;
                esp_now_send(mac_address_ECU_front, (uint8_t *) &rot, sizeof(rot));
