...
static const char *TAG = "HX711"; //TAG for logging

extern "C" uint32_t HX711_ADC::millis() {
    return (uint32_t) (esp_timer_get_time() / 1000);
}

extern "C" uint64_t HX711_ADC::micros() {
    return esp_timer_get_time();
}

#define pinMode(x, y) gpio_set_direction(x, y)
...
extern "C" HX711_ADC::HX711_ADC(gpio_num_t dout, gpio_num_t sck) // constructor
{
	doutPin = dout;
	sckPin = sck;
}

extern "C" void HX711_ADC::setGain(uint8_t gain) // value should be 32, 64 or 128*
{
	if (gain < 64)
		GAIN = 2; // 32, channel B
	else if (gain < 128)
...
extern "C" void HX711_ADC::begin(uint8_t gain)
{
	pinMode(sckPin, OUTPUT);
	pinMode(doutPin, INPUT);
	setGain(gain);
...
extern "C" void HX711_ADC::start(unsigned long t)
{
	t += 400;
 ...
 *	will do conversions continuously for 't' +400 milliseconds (400ms is min. settling time at 10SPS).
 *   Running this for 1-5s in setup() - before tare() seems to improve the tare accuracy. */
extern "C" void HX711_ADC::start(unsigned long t, bool dotare)
{
	t += 400;
	lastDoutLowTime = millis();
	while (millis() < t)
 ...
 *	Will do conversions continuously for 't' +400 milliseconds (400ms is min. settling time at 10SPS).
 *   Running this for 1-5s in setup() - before tare() seems to improve the tare accuracy */
extern "C" int HX711_ADC::startMultiple(unsigned long t)
{
	tareTimeoutFlag = 0;
	lastDoutLowTime = millis();
	if (startStatus == 0)
 ...
 *	Will do conversions continuously for 't' +400 milliseconds (400ms is min. settling time at 10SPS).
 *   Running this for 1-5s in setup() - before tare() seems to improve the tare accuracy */
extern "C" int HX711_ADC::startMultiple(unsigned long t, bool dotare)
{
	tareTimeoutFlag = 0;
	lastDoutLowTime = millis();
	if (startStatus == 0)
...
extern "C" void HX711_ADC::tareNoDelay()
{
	doTare = 1;
	tareTimes = 0;
	tareStatus = 0;
...
extern "C" void HX711_ADC::setCalFactor(float cal)
{
	calFactor = cal;
...
extern "C" bool HX711_ADC::getTareStatus()
{
	bool t = tareStatus;
	tareStatus = 0;
	return t;
...
extern "C" float HX711_ADC::getCalFactor()
{
	return calFactor;
}
...
// else returns 0
extern "C" uint8_t HX711_ADC::update()
{
	int8_t dout = digitalRead(doutPin); // check if conversion is ready
	if (!dout)
	{
		conversion24bit();
		lastDoutLowTime = millis();
...
extern "C" bool HX711_ADC::dataWaitingAsync()
{
	if (dataWaiting)
	{
		lastDoutLowTime = millis();
...
extern "C" bool HX711_ADC::updateAsync()
{
	if (dataWaiting)
	{
		conversion24bit();
...
extern "C" float HX711_ADC::getData() // return fresh data from the moving average dataset
{
	long data = 0;
...
extern "C" long HX711_ADC::smoothedData()
{
	long data = 0;
	long L = 0xFFFFFF;
	long H = 0x00;
...
extern "C" void HX711_ADC::conversion24bit() // read 24 bit data, store in dataset and start the next conversion
{
	...
	{ // read 24 bit data + set gain and start next conversion
		...
		if (SCK_DELAY)
			esp_rom_delay_us(1); // could be required for faster mcu's, set value in config.h
...
extern "C" void HX711_ADC::powerDown()
{
...
extern "C" int HX711_ADC::getReadIndex()
{
	return readIndex;
}
...
extern "C" float HX711_ADC::getConversionTime()
{
...
extern "C" float HX711_ADC::getSPS()
{
...
extern "C" bool HX711_ADC::getTareTimeoutFlag()
{
	return tareTimeoutFlag;
}

extern "C" void HX711_ADC::disableTareTimeout()
{
	tareTimeoutDisable = 1;
}

extern "C" long HX711_ADC::getSettlingTime()
{
...
extern "C" void HX711_ADC::setSamplesInUse(int samples)
{
...
extern "C" bool HX711_ADC::getDataSetStatus()
{
	bool i = false;
	if (readIndex == samplesInUse + IGN_HIGH_SAMPLE + IGN_LOW_SAMPLE - 1)
	{
...
extern "C" float HX711_ADC::getNewCalibration(float known_mass)
{
...
extern "C" bool HX711_ADC::getSignalTimeoutFlag()
{
...
extern "C" void HX711_ADC::setReverseOutput()
{
	reverseVal = true;
}
