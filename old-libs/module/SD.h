...
void init_file(void){
    ...
    while (!SD.begin(hspi->pinSS(), *hspi)){
        INFO("File initialization failed!");
        delay(5000);
    }
    INFO("file initialization done.");
...
void writeFile(fs::FS &fs, const char *path, const char *message){
    xSemaphoreTake(sh_SD, portMAX_DELAY);
        ...
        if(!file)
            ...
            ERROR("writeFile: error opening file");
...
void appendFile(fs::FS &fs, const char *path, const char *message){
    xSemaphoreTake(sh_SD, portMAX_DELAY);
        ...
        if(!file)
            ERROR("appendFile: error opening file");
        
        if(!file.print(message))
            ERROR("appendFile: error appending file");  
